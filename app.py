...
from flask import Flask, render_template, request, redirect, flash, session, url_for, abort, jsonify
...
def buchbutler_request(endpoint, ean):
    ...
    data = response.json()
    return data.get("response") if data and "response" in data else None
...
def send_email(subject, body, recipient=EMAIL_SENDER):
    if not SENDGRID_API_KEY or not EMAIL_SENDER:
        raise ValueError("SENDGRID_API_KEY oder EMAIL_SENDER fehlt!")
    message = Mail(from_email=EMAIL_SENDER, to_emails=recipient, subject=subject, plain_text_content=body)
    try:
        sg = SendGridAPIClient(SENDGRID_API_KEY)
        response = sg.send(message)
        logger.info(f"E-Mail gesendet. Status: {response.status_code}")
...
def checkout():
    if request.method == 'POST':
        ...
        if newsletter:
            try:
                send_email("Neue Newsletter-Anmeldung (Bestellung)", f"{name} ({email}) hat sich beim Checkout für den Newsletter angemeldet.")
            except Exception as e:
                logger.warning(f"Newsletter konnte nicht gesendet werden: {e}")
        flash("Zahlung erfolgreich (Simulation). Vielen Dank!", "success")
...
def newsletter():
    ...
    if not email:
        ...
        return redirect('/')
    try:
        send_email("Neue Newsletter-Anmeldung", f"Neue Anmeldung: {email}")
        flash("Danke! Newsletter erfolgreich.", "success")
...
def neue_bestellung():
    ...
    try:
        ...
        INSERT INTO bestellungen (
            mol_kunde_id, rechnungsadresse_id, mol_zahlart_id, bestelldatum,
            bestellreferenz, seite, bestellfreigabe, mol_verkaufskanal_id,
            ...
            liefer_land_iso, liefer_tel, shipping_option, vormerkung,
            versand_einstellung_id, collectkey, status
        ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            data.get("mol_kunde_id"), data.get("rechnungsadresse_id"), data.get("mol_zahlart_id"),
            data.get("bestelldatum"), data.get("bestellreferenz"), data.get("seite"),
            data.get("bestellfreigabe"), data.get("mol_verkaufskanal_id"),
            data.get("lieferadresse", {}).get("anrede"), data.get("lieferadresse", {}).get("vorname"),
            data.get("lieferadresse", {}).get("nachname"), data.get("lieferadresse", {}).get("zusatz"),
            data.get("lieferadresse", {}).get("strasse"), data.get("lieferadresse", {}).get("hausnummer"),
            data.get("lieferadresse", {}).get("plz"), data.get("lieferadresse", {}).get("ort"),
            data.get("lieferadresse", {}).get("land"), data.get("lieferadresse", {}).get("land_iso"),
            data.get("lieferadresse", {}).get("tel"), data.get("shipping_option"), data.get("vormerkung"),
            data.get("versand_einstellung_id"), data.get("collectkey"), "N"
        ))
        conn.commit()
...
app.register_blueprint(bestellung_api)
